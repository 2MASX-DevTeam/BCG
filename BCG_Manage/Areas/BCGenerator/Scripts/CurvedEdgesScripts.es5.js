// Tool Canvas Card Generator
//
// history:
// - 04/03/2016: added to project
//
// @author Svetlin Krastanov
...
function handleDownAdd(pos) {
    if (!gBezierPath) gBezierPath = new BezierPath(pos);else {
        // If this was probably a selection, change to
        // select/drag mode
        if (handleDownSelect(pos)) return;
...
function render() {
    gBackCtx.clearRect(0, 0, WIDTH, HEIGHT);
    gCtx.clearRect(0, 0, WIDTH, HEIGHT);
    if (gBackgroundImg) gBackCtx.drawImage(gBackgroundImg, 0, 0);
    if (gBezierPath) {
...
function ControlPoint(angle, magnitude, owner, isFirst) {
    ...
    this.setAngle = function (deg) {
        ...
        // TODO fixme fragile
        if (_angle != deg) _angle = deg;
    };

    this.origin = function origin() {
        var line = null;
        if (_isFirst) line = _owner.prev;else line = _owner;
        if (line) return new Point(line.pt.x(), line.pt.y());
        return null;
    };
    ...
    this.translate = function (xDelta, yDelta) {
        ...
        if (my.__proto__.syncNeighbor) updateNeighbor();
    };

    function updateNeighbor() {
        var neighbor = null;
        if (_isFirst && _owner.prev) neighbor = _owner.prev.ctrlPt2;else if (!_isFirst && _owner.next) neighbor = _owner.next.ctrlPt1;
        if (neighbor) neighbor.setAngle(_angle + Math.PI);
    }
...
function LineSegment(pt, prev) {
    ...
    this.draw = function (ctx) {
        ...
        if (my.ctrlPt1) my.ctrlPt1.draw(ctx);
        if (my.ctrlPt2) my.ctrlPt2.draw(ctx);

        // If there are at least two points, draw curve.
        if (my.prev) drawCurve(ctx, my.prev.pt, my.pt, my.ctrlPt1, my.ctrlPt2);
    };

    this.toJSString = function () {
        if (!my.prev) return '  ctx.moveTo(' + Math.round(my.pt.x()) + ' + xoff, ' + Math.round(my.pt.y()) + ' + yoff);';else {
            var ctrlPt1x = 0;
            var ctrlPt1y = 0;
            var ctrlPt2x = 0;
            var ctlrPt2y = 0;
            ...
            return '  ctx.bezierCurveTo(' + ctrlPt1x + ' + xoff, ' + ctrlPt1y + ' + yoff, ' + ctrlPt2x + ' + xoff, ' + ctrlPt2y + ' + yoff, ' + x + ' + xoff, ' + y + ' + yoff);';
        }
    };
    ...
    function init() {
        ...
        if (my.prev) {
            ...
            if (my.prev.pt.x() > my.pt.x()) angle *= -1;
...
function BezierPath(startPoint) {
            ...
            if (current.pathPointIntersects(pos)) {
                ...
                // HEAD CASE
                    ...
                    // TAIL CASE
                            ...
                            if (my.tail) my.tail.next = null;else my.head = null;
                        }
                return true;
            }
            current = current.next;
    ...
    this.toJSString = function () {
        var myString = ['function drawShape(ctx, xoff, yoff) {', '  ctx.beginPath();'];
